 ...
 * the user's `titleBarCompat` pref untouched. This is the `auto` scheme's
 * UA fallback: standard WCO geometry and the `dsh-desktop-titlebar-inset`
 * contract parameter take precedence (titlebar-strip.ts); this only fills in
 * shells that stamp neither.
 */
...
export function Sidebar(props: { ctx: Context; store: SidebarStore }) {
  ...
  // behind.
  //
  // Frameless desktop shells (Electron) that stamp neither WCO nor the
  // `dsh-desktop-titlebar-inset` contract hide the top-right toggle cluster
  // under the native caption. As a LAST-RESORT fallback under the
  // conservative `auto` scheme only, detect such a shell via UA and apply
  // the user's configured strip; every other scheme keeps upstream's exact
  // semantics (web = never, preset/custom = explicit choice).
  ...
  const computedStrip = computeTitleBarStrip(
    desktopEnv, wco, scheme, preset, snapshot.prefs.titleBarStripPx,
  )
  const titleBarStrip = computedStrip > 0 || scheme !== 'auto' || !isFramelessShell()
    ? computedStrip
